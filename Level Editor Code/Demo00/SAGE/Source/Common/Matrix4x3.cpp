...
D3DXMATRIX Matrix4x3::convertMatrix()
{
...
/// \param d Specifies the vector to be assigned.
...
/// \param d Specifies the vector to be assigned as translation.
...
void Matrix4x3::setupRotateX(float theta) {
...
/// \param theta Specifies the angle of rotation.
void Matrix4x3::setupRotateY(float theta) {
...
/// \param s Specifies the scaling factor on each axis.
...
/// \param axis Specifies the axis of scaling.
/// \param k Specifies the scaling factor.
...
/// \param axis Specifies the axis as follows:
///   <ul>
...
///   </ul>
/// \param s Specifies the first shearing coefficient.
/// \param t Specifies the second shearing coefficient.
...
/// \param n Specifies the normal to the plane.
...
///   </ul>
/// \param k Specifies the plane displacement along the axis.
/// \warning Asserts that axis is 1, 2, or 3.  Recommend splitting
///     into three functions, as I see no benefit from specifying
///     the axis numerically and using an assert.
...
/// \param plane Plane to reflect over.  
void Matrix4x3::setupReflect(const Plane &plane)
{
  m11 = -2.0f * plane.a * plane.a + 1.0f;
	m12 = -2.0f * plane.b * plane.a;
	m13 = -2.0f * plane.c * plane.a;

	m21 = -2.0f * plane.a * plane.b;
	m22 = -2.0f * plane.b * plane.b + 1.0f;
	m23 = -2.0f * plane.c * plane.b;

	m31 = -2.0f * plane.a * plane.c;
	m32 = -2.0f * plane.b * plane.c;
	m33 = -2.0f * plane.c * plane.c + 1.0f;
  
  tx = -2.0f * plane.a * plane.d;
  ty = -2.0f * plane.b * plane.d;
  tz = -2.0f * plane.c * plane.d;
...
/// \return The position portion of the matrix.
...
/// \param p Specifies the vector multiplicand.
/// \param m Specifies the matrix multiplicator.
...
/// \param a Specifies the first matrix.
/// \param b Specifies the second matrix.
...
/// \param p Specifies the vector to be transformed.
/// \param m Specifies the matrix multiplicator.
/// \return A reference to the now-modified vector.
Vector3 &operator*=(Vector3 &p, const Matrix4x3 &m) {
	p = p * m;
	return p;
}

/// \param a Specifies the first matrix, which will keep the result.
/// \param b Specifies the second matrix.
/// \return A reference to the first, now-modified matrix.
