/// \file Renderer.cpp
/// \brief Code for the Renderer class.
...
/// \brief Holds information about the current instance (world matrix).
struct InstanceInfo {
...
void	Renderer::setLightEnable(bool flag) {
    ...
    setD3DRenderState(D3DRS_LIGHTING, flag);
...
void Renderer::renderBoundingBox(const AABB3 &box)
{
  ...
  static const unsigned INDEX_BUFFER_INDEXTYPE indices[] = {0,1,1,2,2,3,3,0,4,5,5,6,6,7,7,4,0,4,1,5,2,6,3,7};
...
void	Renderer::renderSprite(float width, float height)
{
  ...
  setLightEnable(false);
	...
	INDEX_BUFFER_INDEXTYPE triList[6] = {0,1,2,0,2,3};
...
void Renderer::renderQuad(RenderVertexL *quad)
{
  ...
  static const INDEX_BUFFER_INDEXTYPE triList[6] = {0,1,2,2,3,0};
...
/// \param boundary Region in which the text should be drawn
...
void Renderer::drawText(const char* text, int x, int y, int fontHandle)
{
  ...
  font->d3dFont->DrawTextA(NULL,text,-1,&boundRect,DT_NOCLIP | DT_LEFT,getARGB());
  return;
}
...
int Renderer::addFont(const char* fontName, int fontWidth, int fontHeight, bool antialiased)
{
	...
	if (font == NULL)
  {
	...
	strcpy_s( FontDesc.FaceName ,sizeof(FontDesc.FaceName), fontName);
...
int	Renderer::findTexture(const char *name) {
  ...
  for (int i = 1 ; i < length ; ++i) 
  {
    if (textureCacheList[i] != NULL)
      if (_stricmp(name, textureCacheList[i]->name.c_str()) == 0) 
      {
        return i;
      }
  }
...
void	Renderer::cacheTexture(TextureReference &texture) {
  ...
  if ((texture.handle >= 0) && (texture.handle < (int)textureCacheList.size())) {
		...
		if (
      (_stricmp(t->name.c_str(), texture.name) == 0) && 
			(t->d3dTexture != NULL)
		) {
